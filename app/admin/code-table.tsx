...
export function CodeTable({
  ...
  useEffect(() => {
    ...
    const columns: ColumnDefinition[] = [
      {
        ...
        formatter: (cell: CellComponent) =>
          `<span class="mono-link">${cell.getValue()}</span>`,
      },
      {
        title: "Business",
        field: "business_name",
        formatter: (cell: CellComponent) =>
          cell.getValue() || '<span class="muted">Unassigned</span>',
      },
      {
        title: "Status",
        field: "status",
        formatter: (cell: CellComponent) =>
          `<span class="status status-${cell.getValue()}">${cell.getValue()}</span>`,
      },
      ...
      },
      {
        title: "Actions",
        formatter: () =>
          `<div class="table-actions"><button type="button" class="view-action" data-action="view" aria-label="View QR code" title="View QR code">${actionIcons.view}</button><button type="button" class="edit-action" data-action="edit" aria-label="Edit code" title="Edit code">${actionIcons.edit}</button><button type="button" class="delete-action" data-action="delete" aria-label="Delete code" title="Delete code">${actionIcons.delete}</button></div>`,
        hozAlign: "right",
        width: 140,
      },
    ];
    table.current = new Tabulator(tableRef.current, {
      data: codes.map((code) => ({
        ...code,
        scan_count: code.scan_events?.length ?? 0,
        last_scanned: code.scan_events?.[0]?.scanned_at ?? "",
      })),
      columns,
      layout: "fitColumns",
      pagination: true,
      paginationSize: 15,
      placeholder: "No codes here yet. Generate your first batch.",
    });
    table.current.on("rowClick", (event: Event, row: RowComponent) => {
      if ((event.target as HTMLElement).closest("button")) return;
      router.push(`/admin/codes/${row.getData().id}`);
    });
    table.current.on(
      "cellClick",
      async (event: UIEvent, cell: CellComponent) => {
        const action = (event.target as HTMLElement).closest("button")?.dataset
          .action;
        const code = cell.getRow().getData() as Code;
        if (action === "view")
          window.open(
            `/admin/codes/${code.id}/qr`,
            "_blank",
            "noopener,noreferrer",
          );
        ...
        if (action === "delete") {
          ...
          if (
            !window.confirm(
              `Delete QR code ${code.short_code}? Its scan history will also be deleted.`,
            )
          )
            return;
          const { error } = await (await import("@/lib/supabase/browser"))
            .createClient()
            .from("qr_codes")
            .delete()
            .eq("id", code.id);
